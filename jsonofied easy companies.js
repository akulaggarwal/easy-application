...
const companiesStr = `[21](https:jobs.lever.co/21) | San Francisco, CA |
...
| [AeroFS](https:www.aerofs.com/company/careers/) | San Francisco, CA |
 ...
 for (let i = 0; i < companiesStr.length; i++) {
   const currentChar = companiesStr[i];
   const lastChar = companiesStr[i - 1];

   if (currentChar === ']') {
     insideArr = false;
     easyCompsObj[name] = {name, website: null, location: null};
     //  console.log(easyCompsObj);
   }
   if (lastChar === '[') {
   ...
   if (currentChar === ')') {
     insideParens = false;
     easyCompsObj[name].website = website;
     website = '';
     name = '';
   }

   if (insideParens) {
     website = website + currentChar;
   }
 }
